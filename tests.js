...
// mocha CSS link
const mochaCSSPath =
  "https://cdnjs.cloudflare.com/ajax/libs/mocha/8.3.2/mocha.min.css";
...
function runTests() {
  ...
  describe("Todo App", function () {
    ...
    it("should mark todo as done by striking through text when todo is clicked ONCE", () => {
      ...
      expect(getComputedStyle(todo).textDecoration).to.not.include(
        "line-through"
      );
      todo.click();
    ...
    it("should add multiple todos", () => {
      const todos = ["banana", "grape", "mango", "apple"];
    ...
    it("should be able to remove todos from middle of the list", () => {
      let todos = ["banana", "grape", "mango", "apple"];
      let todoElements;
      todos.forEach((todo) => {
        input.value = todo;
        button.click();
        todoElements = Array.from(todoList.querySelectorAll("li button"));
        const lastTodo = todoElements.find(
          (todoEl) => todoEl.textContent === todo
        );
        expect(lastTodo).to.exist;
        expect(lastTodo.textContent).to.eq(todo);
      });
      ...
      mangoEl.click();
      expect(getComputedStyle(mangoEl).textDecoration.includes("line-through"))
        .to.be.true;
      mangoEl.click();
      todoElements = Array.from(todoList.querySelectorAll("li button"));
      mangoEl = todoElements.find((todo) => todo.textContent === "mango");
      ...
      todos.forEach((todo) => {
        const todoEl = todoElements.find(
          (todoEl) => todoEl.textContent === todo
        );
